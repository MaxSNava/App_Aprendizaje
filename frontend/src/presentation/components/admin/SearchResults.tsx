...
import { getResultadosVark, getResultadosMbti, descargarReporte } from '../../../api';
...
export const SearchResults = () => {
  ...
  const handleDescargarPDF = async (pruebaId: string) => {
    try {
      // Realizar la solicitud al endpoint de descarga de PDF
      const response = await descargarReporte(pruebaId);
  
      // Crear un enlace temporal para descargar el archivo
      const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `reporte_${pruebaId}.pdf`);
              ...
              <tbody>
                {resultados.map((resultado) => (
                  <tr key={resultado.id} className="hover:bg-gray-50">
                    ...
                    <td className="flex flex-col py-2 px-4 border-b">
                      <button
                      ...
                      <button
                        onClick={() => handleDescargarPDF(resultado.id)}
