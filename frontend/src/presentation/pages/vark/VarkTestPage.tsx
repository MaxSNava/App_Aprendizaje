...
import { getPreguntasVark, sendAnswersVark } from '../../../api';
import { PreguntaVark, PruebaVarkRes } from '../../../types';
...
export const VarkTestPage = () => {
  ...
  const { data: preguntas, isLoading, error } = useQuery({
    queryKey: ['preguntasVark'],
    queryFn: getPreguntasVark,
  });

  const mutation = useMutation({
    mutationFn: (respuestasArray: PruebaVarkRes[]) => sendAnswersVark(pruebaId, respuestasArray),
    onSuccess: () => {
      toast.success('Respuestas enviadas con éxito');
      navigate('/vark/res', { state: { pruebaId } });
    },
    onError: (error) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!pruebaId) {
      toast.error('ID de prueba no encontrado. Por favor, regrese a la página anterior.');
      return;
    }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <h1 className="text-3xl font-bold mb-8 text-center text-blue-600">Test VARK</h1>
      <form className="space-y-8" onSubmit={handleSubmit}>
        {preguntas!.map((pregunta: PreguntaVark, index: number) => (
